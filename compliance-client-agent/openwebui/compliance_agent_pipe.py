"""
title: Jer — Compliance Client Agent
author: asgard
version: 0.1.0
license: MIT
description: >
  Jer routes a chat message to the local compliance gateway, which runs a sandboxed
  Codex agent on a per-chat git worktree of the compliance project and streams
  progress back.
requirements: httpx
"""
...
class Pipe:
    class Valves(BaseModel):
        gateway_url: str = Field(
            default=os.getenv("COMPLIANCE_GATEWAY_URL", "http://127.0.0.1:8642"),
            description="Loopback gateway base URL. Never a public address.",
        )
        gateway_secret: str = Field(
            default=os.getenv("COMPLIANCE_GATEWAY_SECRET", ""),
            description=(
    ...
    def __init__(self) -> None:
        self.type = "pipe"
        self.id = "compliance_agent"
        self.name = "Jer — Compliance Client Agent"
    ...
    def _headers(self) -> dict[str, str]:
        if not self.valves.gateway_secret:
            raise RuntimeError(
                "COMPLIANCE_GATEWAY_SECRET is not set. The pipe will not send "
    ...
    async def pipe(
        ...
        if preview:
            ...
            if not message:
                return "Add the requested compliance change after `/preview`."
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            try:
                ...
                return f"Could not reach the compliance gateway: {exc}"
        ...
        if final.get("preview_url"):
            parts.append(
                ...
                + "\nOpen it, test the requested behavior and downloads, then "
                "return here with anything unexpected."
            )

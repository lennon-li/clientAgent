"""Worktree creation, and the data-directory exclusion check.

The exclusion check is defence-in-depth, not an access-control gate: the client
is internal and already authorized for employee compliance data. What matters
is that a freshly created worktree contains only tracked source, so a client
job starts from a clean, reproducible tree rather than inheriting whatever
happens to be lying in the maintainer's working directory.
"""
...
def test_fresh_worktree_contains_no_gitignored_data_directories(tmp_worktree, cfg):
    """backup/, output/ and library/ are gitignored, so `git worktree add`
    cannot materialise them. Asserted, not assumed."""
...
def test_people_rds_is_absent_from_a_fresh_worktree(tmp_worktree):
    """The specific file that holds real employee records."""
    assert not (tmp_worktree.path / "backup" / "data" / "people.rds").exists()


def test_worktree_contains_the_tracked_package_source(tmp_worktree):
    for expected in ("DESCRIPTION", "NAMESPACE", "R", "tests/testthat.R"):

...
class Worker:
    ...
    def start(self) -> None:
        ...
        COMPLIANCE_WORKER_DISABLED=1 keeps the loop from starting, so API tests
        can exercise queueing and validation without spawning a real Codex
        process (which costs money and edits a worktree).
        """
        if os.environ.get("COMPLIANCE_WORKER_DISABLED") == "1":
            log.warning("worker loop disabled via COMPLIANCE_WORKER_DISABLED")
            return
        self._task = asyncio.create_task(self._run(), name="compliance-worker")
        ...
        if bool(job["preview_requested"]) and status == db.SUCCEEDED:
            try:
                preview_url = await asyncio.to_thread(
                    preview.publish, worktree, self.cfg.preview
                )

#!/usr/bin/env python3
"""Install and enable the reviewed compliance pipe through Open WebUI's API.
...
PIPE = ROOT / "openwebui" / "compliance_agent_pipe.py"
CREDS = Path.home() / "openUI"
BASE = os.environ.get("OPENWEBUI_URL", "http://127.0.0.1:8080").rstrip("/")
FUNCTION_ID = "compliance_agent"
ALLOWED_USER_ID = os.environ.get("COMPLIANCE_ALLOWED_USER_ID", "").strip()
...
def main() -> int:
    try:
        if not ALLOWED_USER_ID:
            raise RuntimeError(
                "COMPLIANCE_ALLOWED_USER_ID is not set. Refusing to grant "
        ...
        form = {
            "id": FUNCTION_ID,
            "name": "Jer — Compliance Client Agent",
            "content": content,
            "meta": {
                "description": "Jer helps clients with compliance-report-related changes and user testing.",
            },
        }
        ...
        if status >= 400:
            ...
            print(
                f"Jer function updated, but model visibility could not be configured "
                f"({status}): {detail}",
                file=sys.stderr,
            )
            return 1
        ...
        print(f"Jer — Compliance Client Agent {action} and enabled.")
        print("Start a new Open WebUI chat and select Jer — Compliance Client Agent.")
        return 0

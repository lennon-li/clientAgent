...
`dataSummary/` is the exception: a git-tracked, de-identified digest of
`agentData/` (column headers, row counts, year ranges, and coarse aggregate
counts -- never row-level content) regenerated by
`tools/generate_data_summary.R` in the source repo. It flows into every client
worktree normally, so the client agent can read it to understand the shape of
the data without ever seeing the real data.

...
def test_gateway_binds_loopback_only(cfg):
    ...
    os.environ["GAMBLING_GATEWAY_HOST"] = "0.0.0.0"
    try:
        with pytest.raises(ValueError, match="loopback"):
            config_mod.load_config()
    finally:
        del os.environ["GAMBLING_GATEWAY_HOST"]
...
def test_gateway_refuses_to_run_without_a_shared_secret(cfg, monkeypatch):
    ...
    monkeypatch.delenv("GAMBLING_GATEWAY_SECRET", raising=False)
    with pytest.raises(RuntimeError, match="GAMBLING_GATEWAY_SECRET"):
        cfg.secret()
...
def test_bootstrap_instruction_forbids_the_dangerous_actions():
    ...
    text = cr.build_bootstrap(
        worktree="/wt", branch="b", default_branch="main", project="gambling",
...
def test_child_environment_does_not_carry_the_gateway_secret(monkeypatch):
    ...
    monkeypatch.setenv("GAMBLING_GATEWAY_SECRET", "leak-me")

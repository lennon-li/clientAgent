...
def test_cannot_write_to_the_service_tree_outside_bound_paths(jailed):
    r = jailed('touch /home/yeli/services/gambling-client-agent/ESCAPED 2>&1; echo "rc=$?"')
    assert "rc=0" not in r.stdout
    assert not Path("/home/yeli/services/gambling-client-agent/ESCAPED").exists()
...
def test_the_maintainer_checkout_and_raw_data_are_not_visible(jailed):
    """The source checkout and its raw contact data stay outside the jail."""
    r = jailed(
        '[ -e /home/yeli/repos/gambling ] && echo CO_VISIBLE || echo CO_ABSENT; '
        '[ -r /home/yeli/repos/gambling/data/contacts.csv ] '
        '&& echo DATA_READABLE || echo DATA_BLOCKED'
    )
    assert "CO_ABSENT" in r.stdout
    assert "DATA_BLOCKED" in r.stdout
...
def test_quarto_is_available_inside_the_jail(jailed):
    """The agent must be able to validate dashboard tooling while contained."""
    r = jailed('/home/yeli/.local/quarto-1.5.56/bin/quarto --version 2>&1', timeout=300)
    assert r.returncode == 0, f"Quarto unavailable inside the jail: {r.stdout}{r.stderr}"


@pytest.mark.slow
def test_quarto_check_passes_inside_the_jail(jailed):
    """The project's supported toolchain check remains usable while contained."""
    r = jailed('/home/yeli/.local/quarto-1.5.56/bin/quarto check 2>&1', timeout=900)
    assert r.returncode == 0, f"quarto check failed in jail:\n{r.stdout[-2000:]}"
...
def test_memory_is_not_sourced_from_the_synced_vault(cfg):
    """Gambling has no remote project memory; it must not point at obsidian."""

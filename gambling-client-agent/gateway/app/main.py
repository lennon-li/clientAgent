...
log = logging.getLogger("gambling.gateway")
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    if not logging.getLogger("gambling").handlers:
        logging.basicConfig(
            level=logging.INFO,
            format="%(asctime)s %(levelname)s %(name)s: %(message)s",
        )
        logging.getLogger("gambling").setLevel(logging.INFO)
...
def create_app(cfg: Config | None = None) -> FastAPI:
    ...
    app = FastAPI(
        title="Gambling Client Agent Gateway",
        version="0.1.0",
        lifespan=lifespan,
    )
    app.state.cfg = cfg

...
from $GAMBLING_AGENT_HOME, falling back to the parent of the directory
holding config.yaml. Relative paths in config.yaml are joined to that root, so
`cp -a` to /srv/gambling-client-agent under a different UID needs no edits.
"""
...
def resolve_home(config_path: Path) -> Path:
    ...
    $GAMBLING_AGENT_HOME wins; otherwise the tree root is the parent of the
...
def load_config(path: str | os.PathLike[str] | None = None) -> Config:
    ...
    g = data["gateway"]
    host = os.environ.get("GAMBLING_GATEWAY_HOST", str(g["host"]))
    ...
    port = int(os.environ.get("GAMBLING_GATEWAY_PORT", g["port"]))
    ...
    preview_target = _path(
        home,
        os.environ.get(
            "GAMBLING_PREVIEW_DIR",
            str(preview.get("target_dir", "/srv/shiny-server/test/gambling")),
        ),
    )
    if preview_target == Path("/srv/shiny-server/gambling"):
        raise ValueError("preview target may not be the live gambling app")

    provider = os.environ.get(
        "GAMBLING_AGENT_PROVIDER", str(c.get("provider", "codex"))
    ).strip().lower()
    ...
    binary_env = (
        "GAMBLING_COPILOT_BIN" if provider == "copilot"
        else "GAMBLING_CODEX_BIN"
    )
    path_env = (
        "GAMBLING_COPILOT_PATH" if provider == "copilot"
        else "GAMBLING_CODEX_PATH"
    )
    ...
    return Config(
        ...
        project=ProjectConfig(
            ...
            source_repo=_path(
                home, os.environ.get("GAMBLING_SOURCE_REPO", p["source_repo"])
            ),
            ...
            memory_index=str(
                _path(
                    home,
                    os.environ.get(
                        "GAMBLING_MEMORY_INDEX", str(p["memory_index"])
                    ),
                )
            ),

...
Client branches (`client/gambling/*`) exist only in the service clone and are
...
def git_env() -> dict[str, str]:
    ...
    return {
        "PATH": os.environ.get("GAMBLING_GIT_PATH", _MIN_PATH),
...
def check_excluded_paths(
    ...
    authorized for employee gambling data, so a hit here is reported and
...
def commit_all(
    worktree: str | Path, message: str, author: str = "gambling-agent <agent@localhost>"
) -> str | None:
